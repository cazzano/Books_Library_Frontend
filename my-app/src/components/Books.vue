<template>
  <div class="books-container">
    <h1
      class="text-3xl font-bold text-center mb-8 font-arabic text-emerald-800"
    >
    ...
    <div v-if="loading" class="flex justify-center items-center py-20">
      <div
        class="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-emerald-700"
      ></div>
    </div>
    ...
    >
      ...
      >
        ...
        <div class="p-5">
          <h2 class="text-xl font-bold text-emerald-800 mb-2">
            {{ book.name }}
          </h2>
    ...
    <div
      v-else
      class="bg-blue-50 border border-blue-300 text-blue-700 px-6 py-12 rounded-lg text-center"
    >
    ...
    <!-- Page info -->
    <div
      v-if="filteredBooks.length > 0 && !loading && !error"
    ...
    <div
      v-if="selectedBook"
      class="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-60 p-4"
    >
      <div
        class="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-fadeIn"
      >
        <div class="relative">
          <img
        ...
        <div class="p-6">
          <h2 class="text-2xl font-bold text-emerald-800 mb-3">
            {{ selectedBook.name }}
          </h2>
...
export default {
  ...
  data() {
    return {
      ...
      categories: [],
      fallbackImage:
        "https://via.placeholder.com/400x250?text=Book+Cover+Not+Available",
      currentPage: 1,
      booksPerPage: 10,
    };
  },
  ...
  meta: {
    ...
    meta: [
      {
        property: "og:title",
        content: "Islamic Books | Al-Baraka Islamic Center",
      },
      {
      ...
      {
        name: "keywords",
        content: "Islamic books, Quran, Hadith, Fiqh, Islamic manuscripts",
      },
    ],
  },
    ...
    filteredBooks() {
      return this.books.filter((book) => {
        ...
        const matchesCategory =
          this.categoryFilter === "" ||
          book.category.toLowerCase() === this.categoryFilter.toLowerCase();
    ...
    async fetchBooks() {
      ...
      try {
        const response = await fetch(
          "https://bookslibrarybackend-production.up.railway.app/books",
        );
        if (!response.ok) {
